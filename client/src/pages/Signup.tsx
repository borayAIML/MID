    ...
    try {
      ...
      if (response && response.success) {
        ...
        toast({
          ...
          description: response && response.message ? response.message : "An error occurred during registration. Please try again.",
          variant: "destructive",
        });
      }
    } catch (error) {
      ...
      <Card className="w-full max-w-md shadow-lg border-indigo-100">
        ...
        <CardFooter className="flex justify-center">
          <p className="text-sm text-muted-foreground">
            ...
            <a onClick={() => navigate("/login")} className="text-indigo-600 hover:text-indigo-800 cursor-pointer font-medium">
              Sign in instead
            </a>
          </p>
        </CardFooter>
