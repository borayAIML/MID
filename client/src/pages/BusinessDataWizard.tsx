...
export default function BusinessDataWizard({
  userData,
  onComplete,
}: BusinessDataWizardProps) {
      ...
      if (!companyId) {
        ...
        if (newCompanyId) {
          ...
          toast({
            title: "Warning",
            description:
              "There was an issue setting up your company. Some features may not work properly.",
            variant: "destructive",
          });
        }
      } else {
  ...
  const createCompany = async (): Promise<number | null> => {
    ...
    try {
      ...
      console.log(
        "Using hardcoded company ID as last resort:",
        hardcodedCompanyId,
      );
      ...
      if (!currentCompanyId && userData) {
        ...
        if (!currentCompanyId) {
          ...
          toast({
            title: "Warning",
            description:
              "Company record could not be created. Your data may not be saved.",
            variant: "destructive",
          });
          goToNextStep();
          return;
      ...
      if (currentCompanyId) {
        ...
        await apiRequest("/api/financials", {
          ...
          body: JSON.stringify({
            ...
            revenueCurrent:
              data.revenueCurrent !== null
                ? data.revenueCurrent.toString()
                : null,
            revenuePrevious:
              data.revenuePrevious !== null
                ? data.revenuePrevious.toString()
                : null,
            revenueTwoYearsAgo:
              data.revenueTwoYearsAgo !== null
                ? data.revenueTwoYearsAgo.toString()
                : null,
            ebitda: data.ebitda !== null ? data.ebitda.toString() : null,
            netMargin:
              data.netMargin !== null ? data.netMargin.toString() : null,
          }),
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const handleDocumentStepComplete = async (data: typeof documentData) => {
    ...
    try {
      ...
      if (currentCompanyId) {
        ...
        const uploadPromises = Object.entries(data).map(async ([key, file]) => {
          if (file) {
            ...
            formData.append(
              "type",
              key.replace("Statements", "").replace("Documents", ""),
            );
      ...
      toast({
        title: "Error",
        description:
          "Failed to complete document upload process. You can still proceed.",
        variant: "destructive",
      });
    }
  ...
  const handleOwnerIntentStepComplete = async (
    data: typeof ownerIntentData,
  ) => {
    ...
    try {
      ...
      if (!currentCompanyId && userData) {
        ...
        if (!currentCompanyId) {
          ...
          toast({
            title: "Error",
            description:
              "Company information is missing. Please restart the process from the beginning.",
            variant: "destructive",
          });
          return;
        }
      ...
      if (!currentCompanyId) {
        toast({
          title: "Error",
          description:
            "Company information is missing. Please restart the process.",
          variant: "destructive",
        });
        return;
      }
      ...
      const ownerIntentResponse = await apiRequest("/api/owner-intent", {
        ...
        body: JSON.stringify({
          ...
          valuationExpectations:
            data.valuationExpectations !== null
      ...
      const valuationResponse = await apiRequest(
        `/api/companies/${currentCompanyId}/generate-valuation`,
        {
          method: "POST",
          body: JSON.stringify({}),
          headers: {
      ...
      toast({
        title: "Error",
        description:
          "Failed to generate valuation. Please check that all required data has been provided.",
        variant: "destructive",
      });
    }
  };
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 text-center">
          ...
          <p className="mt-2 text-gray-600 max-w-xl mx-auto">
            Complete this 5-step process to receive your instant AI-powered
            valuation
                ...
                >
                  ...
                  >
                    {isCompleted ? (
                      ...
                      <StepIcon
                        className={`w-5 h-5 md:w-6 md:h-6 ${isActive ? "text-white" : step.color}`}
                      />
                    )}
                  ...
                  <div
                    className={`mt-2 text-xs font-medium hidden sm:block ${isActive ? "text-primary" : isCompleted ? "text-green-500" : "text-gray-500"}`}
                  >
                    {isCompleted ? (
        ...
        <Card className="shadow-lg border border-gray-200 overflow-hidden relative">
          ...
          {isGeneratingValuation && (
            <div className="absolute inset-0 bg-white bg-opacity-90 z-50 flex flex-col items-center justify-center">
              <ValuationLoader
                text="Generating your business valuation..."
                loops={3}
              />
              <p className="mt-4 text-gray-600 max-w-md text-center">
                Our AI is analyzing 57+ data points to determine your company's
                true market value...
              </p>
            </div>
          )}
        ...
        <div className="mt-4 text-center">
          <p className="text-xs text-gray-400">
            <span className="inline-block animate-pulse relative">●</span> 236
            business owners completed their valuation this week
          </p>
        </div>
      </div>
    </div>
